...
def generate_image_description(image_bytes, model_name="ministral-3b"):
    ...
    try:
        ...
        # Using official ollama python client which handles base64 streaming efficiently over HTTP
            ...
            messages=[
                {
                    "role": "user",
                    "content": prompt,
                    "images": [image_bytes]  # ollama SDK handles binary format encoding natively
                }
            ]
        )
...
def load_documents(uploaded_files, vlm_model="ministral-3b"):
    ...
    for uploaded_file in uploaded_files:
        ...
        try:
            if file_ext == '.pdf':
                use_ocr_mode = "ocr" in vlm_model.lower()
                ...
                for page_num in range(len(pdf_document)):
                    ...
                    if use_ocr_mode:
                        # Deepseek-OCR handles huge images poorly natively. Using matrix=1,1 for 72DPI.
                        # Do not double the resolution unless absolutely necessary.
                        pix = page.get_pixmap(matrix=fitz.Matrix(1, 1))
                        image_bytes = pix.tobytes("png")
                        image_ext = "png"

                        image_filename = f"{os.path.splitext(uploaded_file.name)[0]}_page_{page_num+1}_fullpage.{image_ext}"
                        ...
                        st.write(f"Debug OCR Page {page_num+1}:")
                        ...
                        image_doc = Document(
                            page_content=f"OCR Text (Page {page_num+1}): {description}",
                            metadata={
                                "source": uploaded_file.name,
                                "page": page_num + 1,
                                "type": "ocr_page",
                        ...
                        st.toast(f"OCR processed full page {page_num+1}")
                    else:
...
def main():
    ...
    with st.sidebar:
        ...
        vision_keywords = ["ministral", "llava", "moondream", "gemma", "qwen", "llama3.2-vision", "deepseek", "ocr"]
            ...
            if len(uploaded_files) > 5:
                ...
                if st.button("Process Documents (with Vision)", type="primary"):
                    ...
                    with st.spinner(f"Ingesting documents and analyzing images with {selected_vision_model}..."):
                        # 1. Load
                        st.session_state.processed_images = []
                        raw_docs, loaded_tabular_data = load_documents(uploaded_files, vlm_model=selected_vision_model)
                        if not raw_docs:
        ...
        if uploaded_image:
            ...
            if "current_image_id" not in st.session_state or st.session_state.current_image_id != img_id:
                ...
                with st.sidebar.status(f"Analyzing image with {selected_vision_model}...", expanded=True):
                    img_bytes = uploaded_image.getvalue()
                    desc = generate_image_description(img_bytes, model_name=selected_vision_model)
